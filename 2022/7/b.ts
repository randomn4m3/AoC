...
const unused = 70_000_000 - folderSize.get("/");
const needToFree = 30_000_000 - unused;
console.log(needToFree);

const smallest = { value: 30_000_000, pwd: "" };

if (needToFree > 0) {
  folderSize.forEach((v, k) => {
    if (((v - needToFree) > 0) && (v < smallest.value)) {
      smallest.pwd = k;
      smallest.value = v;
    }
  });
